...
class Readout(nn.Module):
    ...
    def __init__(self, embedding_dim, hidden_dim, output_dim, n_layers, num_cats, activation, graph_class = True):
...
class GAT(torch.nn.Module):
    ...
    def __init__(self, num_of_layers, num_heads_per_layer, num_features_per_layer, num_classes, num_layers_readout, add_skip_connection=True, bias=True,
        ...
        self.readout = Readout(num_features_per_layer[-1], num_features_per_layer[-1], num_features_per_layer[-1], n_layers=num_layers_readout, num_cats=num_classes, activation=nn.ReLU)
    ...
    def forward(self, adj, x, output_hidden_states = False, output_z = False):

        adj_norm = torch.where(adj == 0, -torch.inf, 0.)
        hidden_states = []
        z = torch.tensor([]).cuda()
        hidden_state = x
        for i, layer in enumerate(self.gat_net):
            hidden_state = layer((hidden_state, adj_norm))
            if i == 0 and output_z:
                z = hidden_state[2]
            hidden_state = hidden_state[0]
            if output_hidden_states:
                hidden_states.append(hidden_state.clone())
        
        return {'logits': self.readout((hidden_state, adj_norm)), 'hidden_states': hidden_states, 'z': z}
    
    def get_features(self, adj, x):
        return self(adj, x, output_hidden_states = True)['hidden_states']
...
class GATLayerImp3(GATLayer):
    ...
    def forward(self, data):
        ...
        return (out_nodes_features, edge_index, nodes_features_proj)
...
class GATLayerImp2(GATLayer):
    ...
    def forward(self, data):
        ...
        return (out_nodes_features, connectivity_mask, nodes_features_proj)
    ...
    def forward_batched(self, data):
        ...
        assert connectivity_mask.shape == (in_nodes_features.shape[0], num_of_nodes, num_of_nodes), \
            f'Expected connectivity matrix with shape=({in_nodes_features.shape[0]},{num_of_nodes},{num_of_nodes}), got shape={connectivity_mask.shape}.'
        ...
        # Apply the scoring function (* represents element-wise (a.k.a. Hadamard) product)
        # shape = (B, N, NH, FOUT) * (1, 1, NH, FOUT) -> (B, N, NH, 1)
        # Optimization note: torch.sum() is as performant as .sum() in my experiments
        scores_source = torch.sum((nodes_features_proj * self.scoring_fn_source.unsqueeze(0)), dim=-1, keepdim=True)
        scores_target = torch.sum((nodes_features_proj * self.scoring_fn_target.unsqueeze(0)), dim=-1, keepdim=True)

        # src shape = (B, NH, N, 1) and trg shape = (B, NH, 1, N)
        scores_source = scores_source.transpose(1, 2)
        scores_target = scores_target.permute(0, 2, 3, 1)

        # shape = (B, NH, N, 1) + (B, NH, 1, N) -> (B, NH, N, N) with the magic of automatic broadcast <3
        # In Implementation 3 we are much smarter and don't have to calculate all NxN scores! (only E!)
        # Tip: it's conceptually easier to understand what happens here if you delete the NH dimension
        all_scores = self.leakyReLU(scores_source + scores_target)
        # connectivity mask will put -inf on all locations where there are no edges, after applying the softmax
        # this will result in attention scores being computed only for existing edges
        all_attention_coefficients = self.softmax(all_scores + connectivity_mask.unsqueeze(1))
        all_attention_coefficients = torch.where(all_attention_coefficients.isnan(), 0., all_attention_coefficients)
        #
        # Step 3: Neighborhood aggregation (same as in imp1)
        #

        # batch matrix multiply, shape = (B, NH, N, N) * (B, NH, N, FOUT) -> (B, NH, N, FOUT)
        out_nodes_features = torch.bmm(
            all_attention_coefficients.reshape(-1, num_of_nodes, num_of_nodes), 
            nodes_features_proj.transpose(1, 2).reshape(-1, num_of_nodes, self.num_out_features)
        ).reshape(-1, self.num_of_heads, num_of_nodes, self.num_out_features)
        ...
        if self.add_skip_connection:  # add skip or residual connection
            if out_nodes_features.shape[-1] == in_nodes_features.shape[-1]:  # if FIN == FOUT
                # unsqueeze does this: (B, N, FIN) -> (B, N, 1, FIN), out features are (N, NH, FOUT) so 1 gets broadcast to NH
                # thus we're basically copying input vectors NH times and adding to processed vectors
        ...
        return (out_nodes_features, connectivity_mask, nodes_features_proj) if self.activation is None else (self.activation(out_nodes_features), connectivity_mask, nodes_features_proj)
